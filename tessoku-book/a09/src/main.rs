...
fn main() {
    ...
    input! {
        ...
        w: usize,
        n: usize,
        abcd_n: [(usize, usize, usize, usize); n]
    }

    // exercise
    let mut kingdom: Vec<Vec<i64>> = vec![vec![0; w + 2]; h + 2];
    // 予想から王国に+1, -1の反映
    for (a, b, c, d) in abcd_n {
        kingdom[a][b] += 1;
        kingdom[a][d + 1] += -1;
        kingdom[c + 1][b] += -1;
        kingdom[c + 1][d + 1] += 1;
    ...
    for i in 1..=h {
        for j in 1..=w {
            kingdom[i][j] = kingdom[i][j] + kingdom[i][j - 1];
        }
    }
    // 累積和を計算 ↓
    for i in 1..=h {
        for j in 1..=w {
            kingdom[i][j] = kingdom[i][j] + kingdom[i - 1][j];
        }
    }
    // 答え
    for i in 1..=h {
