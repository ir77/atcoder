...
fn main() {
    ...
    // 普通に解くとTLEになる
    // for (l, r) in lr_d {
    //     let a_n_lr = [&a_n[0..(l - 1)], &a_n[r..n]].concat();
    //     println!("{}", a_n_lr.iter().max().unwrap());
    // }

    // 左から右の累積maxを求める
    let mut a_n_max_left_to_right: Vec<usize> = vec![0; n];
    for i in 0..n {
        if i == 0 {
            a_n_max_left_to_right[i] = a_n[i];
        } else {
            a_n_max_left_to_right[i] = max(a_n[i], a_n_max_left_to_right[i-1]) ;
        }
    }
    // 右から左の累積maxを求める
    let mut a_n_max_right_to_left: Vec<usize> = vec![0; n];
    for i in (0..n).rev() {
        if i == n - 1 {
            a_n_max_right_to_left[i] = a_n[i];
        } else {
            a_n_max_right_to_left[i] = max(a_n[i], a_n_max_right_to_left[i+1]) ;
        }
    }
    for (l, r) in lr_d {
        println!("{}", max(a_n_max_left_to_right[l-2], a_n_max_right_to_left[r]))
    }
}
