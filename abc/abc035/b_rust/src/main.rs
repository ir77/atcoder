...
fn main() {
    ...
    let mut drone_position: (i64, i64) = (0, 0);
    let mut question_count = 0;
    for i in s {
        match i {
            'L' => drone_position.0 -= 1,
            'R' => drone_position.0 += 1,
            'U' => drone_position.1 += 1,
            'D' => drone_position.1 -= 1,
            '?' => question_count += 1,
            _ => (),
        }
    }
    let drone_abs = drone_position.0.abs() + drone_position.1.abs();
    if t == 1 {
        println!("{}", drone_abs + question_count);
    } else {
        if drone_abs > question_count {
            println!("{}", drone_abs - question_count);
        } else {
            let diff = question_count - drone_abs;
            println!("{}",  diff % 2);
        }
    }
}
