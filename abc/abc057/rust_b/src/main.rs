...
fn main() {
    ...
    // exercise
    for student in ab {
        let mut answer_diff: i64 = 0;
        let mut answer_index: usize = 0;

        for (index, checkpoint) in cd.iter().enumerate() {
            let diff = (student.0 - checkpoint.0).abs() + (student.1 - checkpoint.1).abs();
            if index == 0 || diff < answer_diff {
                answer_index = index;
                answer_diff = diff;
            }
        }
        println!("{}", answer_index + 1);
    }
}
