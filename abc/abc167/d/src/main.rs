...
fn main() {
    ...
    let k = input[1];
    let towns: Vec<usize> = get_vec_input();
    ...
    for i in 0..k {
        let next_town = towns[history.last().unwrap() - 1];
        if visited_towns.contains(&next_town) {
            let index = history.iter().position(|&x| x == next_town).unwrap();
            let diff = history.len() - index;
            let rest = (k - i) % diff;

            if rest == 0 {
                println!("{}", history.last().unwrap());
            } else {
                println!("{}", history[history.len() - (diff - rest) - 1]);
            }
            return;
        } else {
            history.push(next_town);
            visited_towns.insert(next_town);
        }
    }
