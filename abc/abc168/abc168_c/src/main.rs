...
fn main() {
    ...
    let a_angle = ((h + m / 60.0) / 12.0) * 360.0;
    let b_angle = (m / 60.0) * 360.0;

    let diff_angle = (a_angle - b_angle).abs();
    let diff_angle = if diff_angle > 180.0 {
        360.0 - diff_angle
    } else {
        diff_angle
    };
    if diff_angle == 180.0 {
        ...
        let answer = (a * a + b * b - 2.0 * a * b * get_radian_from(diff_angle).cos()).sqrt();
        println!("{}", answer);
    }
}
