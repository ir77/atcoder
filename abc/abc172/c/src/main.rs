...
fn main() {
    ...
    let m = input[1];
    let k = input[2];
    let a: Vec<u128> = get_vec_input();
    let b: Vec<u128> = get_vec_input();

    let mut a2: Vec<u128> = vec![0; 1];
    for i in 0..n {
        let value = a[i] + a2[i];
        if value > k as u128 {
            break;
    ...
    // leftとrightのindexを動かしながら答えを探る
    let mut answer = a2.len() - 1;
    let mut left = a2.len() - 1;
    let mut right = 0usize;
    let k = k as u128;

    while right != b2.len() - 1 {
        right += 1;
        while a2[left] + b2[right] > k {
            if left > 0 {
                left -= 1;
            } else {
                break;
        ...
        if a2[left] + b2[right] <= k && left + right > answer {
            answer = left + right;
        }
    }
