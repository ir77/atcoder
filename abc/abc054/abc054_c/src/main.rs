...
fn main() {
    ...
    loop {
        ...
        if !pattern.next_permutation() || pattern[0] != 1 {
            break;
        }
    }
    let lines = (0..m).map(|_| get_vec_input()).collect::<Vec<Vec<usize>>>();

    // do
    // pattern毎に移動が可能かを判定する
    let mut answer = 0;
    for pattern in patterns {
        for i in 1..n {
            let from = pattern[i - 1];
            let to = pattern[i];
            let mut out_count = 0;
            for line in &lines {
                if (line[0] != from || line[1] != to) && (line[1] != from || line[0] != to) {
                    out_count += 1
                }
            }
            if out_count == lines.len() {
                break;
            }
            if i == n - 1 {
                answer += 1;
