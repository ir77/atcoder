...
fn main() {
    // setup
    let mut n: i64 = get_vec_input()[0];
    let alphabet = (0..26).map(|x| (x + b'a') as char).collect::<Vec<_>>();

    let mut answer: Vec<char> = Vec::new();
    while n > 0 {
        let number = (n - 1) % 26;
        answer.push(alphabet[number as usize]);
        n -= number;
        let number = n / 26;
        n = number;
    }
    answer.iter().rev().for_each(|x| print!("{}", x));
}
